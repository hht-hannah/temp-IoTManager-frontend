    ...
    <div class="search-container">
      <el-form :inline="true" class="header">
        ...
        <el-form-item v-if="checkTypeAuth(['CONFIGURE_SYSTEM_CREATE'])">
          <el-button type="primary" @click="deviceTypeAddVisible = true">添加</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div
      class="table-container"
      v-if="checkTypeAuth(['CONFIGURE_SYSTEM_RETRIEVE'])"
      style="width: 60%"
    >
      <el-table
        v-loading="loading"
      ...
      >
        ...
        <el-table-column prop="offlineTime" label="超时告警时间" sortable="custom"></el-table-column>
        <el-table-column
          fixed="right"
          label="操作"
          v-if="checkTypeAuth(['CONFIGURE_SYSTEM_DELETE', 'CONFIGURE_SYSTEM_UPDATE'])"
        >
          <template slot-scope="scope">
            <el-button
              ...
              type="text"
              size="small"
              v-if="checkTypeAuth(['CONFIGURE_SYSTEM_UPDATE'])"
            >修改</el-button>
            <el-button
              @click="deleteDeviceType(scope.row)"
              type="text"
              size="small"
              v-if="checkTypeAuth(['CONFIGURE_SYSTEM_DELETE'])"
            >删除</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  methods: {
    ...
    async getTableData() {
      ...
      const deviceTypeName =
        this.searchDeviceType === "全部" ? "all" : this.searchDeviceType;
      const data = await getDetailedDeviceType(
        "search",
        this.curPage,
        searchColumn,
        searchOrder,
    ...
    async deleteDeviceType(row) {
      const affiliateDevice = (await getDeviceTypeAffiliateDevice(row.id)).data
        .d;
    ...
    async update(formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          try {
            const data = await updateDeviceType(
              this.updateData.id,
              this.updateData
            );

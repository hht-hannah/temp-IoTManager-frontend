<template>
  <div>
    ...
    <div
      ...
      v-if="checkPropertyAuth(['CONFIGURE_SYSTEM_RETRIEVE'])"
      style="width: 60%;"
    >
      <el-table :data="properties" border @sort-change="sortChange">
        <el-table-column prop="device" label="所属设备"></el-table-column>
        <el-table-column prop="fieldId" label="属性ID"></el-table-column>
        ...
        <el-table-column fixed="right" label="操作">
          <template
            slot-scope="scope"
            v-if="checkPropertyAuth(['CONFIGURE_SYSTEM_DELETE', 'CONFIGURE_SYSTEM_UPDATE'])"
          >
            <el-button
              @click="openUpdateForm(scope.row)"
              type="text"
              size="small"
              v-if="checkPropertyAuth(['CONFIGURE_SYSTEM_UPDATE'])"
            >修改</el-button>
            <el-button
              @click="deleteField(scope.row)"
              type="text"
              size="small"
              v-if="checkPropertyAuth(['CONFIGURE_SYSTEM_DELETE'])"
            >删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async getAllProperties() {
      ...
      const searchColumn =
        this.curSortColumn === "" ? "device" : columnMap[this.curSortColumn];
      const searchOrder =
        this.curOrder === "" ? "asc" : orderMap[this.curOrder];
      const property =
        this.searchProperty === "全部" ? "all" : this.searchProperty;
